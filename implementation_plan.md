...
- **Advanced Anti-Piracy Layer**:
  - **Native Shield**: Implement native code to detect and block screen recording/screenshots (FLAG_SECURE on Android, screen recording detection on iOS).
  - **Dynamic Watermarking**: Create a Flutter overlay that displays user-specific info (email/IP) at random positions to deter recording with external cameras.
  - **YouTube/Vimeo Stealth Mode**: Disable all player branding and external links. 
- **Secure Offline Learning (Rural Connectivity)**:
  - **Encrypted Local Storage**: Save downloaded lessons in a hidden, encrypted app data folder (not accessible by file managers).
  - **Offline Policy**: 
    - Max 5 concurrent offline lessons per course.
    - 7-day auto-expiration (local metadata checks).
    - Automatic deletion of expired files to prevent unauthorized long-term access.
  - **Hacker-Proofing**: Use encrypted file naming and internal database tracking so raw video files cannot be identified or played outside the Aurado app.
